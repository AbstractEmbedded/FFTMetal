...
@synthesize layerSizeDidUpdate = _layerSizeDidUpdate;
...
-(void)checkForLayerUpdates
{
    if(_layerSizeDidUpdate)
    {
        ...
        if( self.delegate && [_delegate respondsToSelector:@selector(reshape:)] )
...
- (void)display
{
    ...
    @autoreleasepool
    {
        ...
        [self checkForLayerUpdates];
        
        // rendering delegate method to ask renderer to draw this frame's content
        if( self.delegate && [_delegate respondsToSelector:@selector(render:)] )
